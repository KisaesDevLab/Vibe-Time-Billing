...
`),
  },
  {
    ...
    summary:
      'Bulk-create or update clients from a CSV or Excel file (UltraTax Data Mining exports upload as-is): preview create/update/skip per row, then import.',
...
2. On **1 · Upload**, either click **Download CSV template** (saves \`client-import-template.csv\`) to get the exact header row and fill it in, or use a tax-software export as-is (see *UltraTax* below). Choose your file under **CSV or Excel file**.
...
6. Review the summary — **Total rows**, **Will create**, **Will update**, **Will skip** — and the per-row table: **Row**, **Name**, **Action** (create / update / skip), **Owner** (the resolved staff member, or *(default)*), **People** (how many contacts the row carries), **Changes** (which fields an update will rewrite), **Reason / warnings**.
7. Click **Import {n} new (+ update {m})** (**Importing…** while it runs). On completion you'll see *"Imported {n} clients."* plus updated / people-linked / skipped counts; click **Done**. Use **Back** to return to upload.

## UltraTax CS Data Mining export
An UltraTax **Data Mining** export (columns such as *Client ID*, *Client name*, *Client name (first last)*, *Contact address 1*, *Filing status*, *Federal entity type*, *1040, Tp first name*, *Contact, Tp email address*, *Contact, Mobile telephone number*, *1040, Sp first name*, *Contact, Sp email address*, *Preparer name*) can be uploaded without editing:
- **Client ID** becomes the client's ID (the **External ID** field); re-uploading a later export matches on it.
- **Client name** → client name; **Client name (first last)** → client-facing name; **Filing status** words (*Married filing joint*, *Head of household*…) → MFJ / HOH etc.; **Federal entity type** \`I\` → Individual.
...
- **Preparer name** becomes the client **owner** when it matches a staff member's name (middle initials and punctuation are ignored); otherwise the **Default client owner** is used and the row shows an *owner not matched → default* warning.

## Field reference
- **Required column:** \`name\` (or UltraTax *Client name*) — every other column is optional. Columns are matched by header name; unknown columns are ignored.
- Client columns: \`name\`, \`client_facing_name\`, \`client_owner_email\` / \`client_owner_name\` (also \`preparer_name\`), \`office\`, \`client_type\`, \`entity_type\` (enum names or codes I/S/C/P/F/X), \`external_id\` (the client id — your own or UltraTax *Client ID*), \`filing_status\`, \`pipeline_stage\`, \`terms_days\`, \`invoice_consolidation_preference\`, \`tags\`, \`mailing_street1\`, \`mailing_street2\`, \`mailing_city\`, \`mailing_state\`, \`mailing_postal\`, \`mailing_country\`.
...
Rows that match an existing client — by \`external_id\` (Client ID), then name — are marked **update**: new people are linked to the client; with **Update fields on existing clients** ticked the **Changes** column lists the columns that will be rewritten (the owner is only changed when the row's owner/preparer matches a staff member). Rows with problems are marked **skip** with a **Reason** (missing name, unknown office, invalid filing status, duplicate id in the file…). The preview must be run before **Import** is meaningful (otherwise it prompts *"Run the preview first."*).

## Adding or linking a taxpayer or spouse by hand
On a client, open the **People** card → **Add contact**. Start typing a name or email: pick an existing person from the firm directory to **link** them (no duplicate record), or type a new name to create one. Choose the **Role** (Individual clients pre-select *Taxpayer* for the first person and *Spouse* for the next) and mark the taxpayer as **Primary** / **Billing** from the row's manage panel.
