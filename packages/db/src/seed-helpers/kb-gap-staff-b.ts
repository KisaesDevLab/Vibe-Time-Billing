...
4. Pick **Deliver by** — **Email**, **Text**, or **Email + text** — then click **Send for signature**. Each signer gets their link and the request moves to *Sent*. Texting needs a mobile on the signer; anyone without one is named in a warning before you send, and a sequential (in-order) request keeps using the same channel for each later signer.
...
- **Completion confirmation** — when the last signer finishes, the client's billing (or primary) contact gets a confirmation carrying a secure download link for the signed PDF (the recipient enters an emailed access code, then downloads; the link expires after 30 days). When there's no filed copy to link — no client on the request, or the client's folder isn't bound — the email instead names who to contact. That sentence is the \`{{ document.copy_note }}\` variable in **Admin → Notification templates → Signature completed**.
