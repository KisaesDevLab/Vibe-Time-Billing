...
// =====================================================================
// 0074 — Proposals + Live Agreements (PP0 — first stage of §2.8)
//
// proposal             — header + signature provenance
// proposal_tier        — 1..4 picker cards
// proposal_addon       — required + optional line items
// agreement            — UNIQUE per signed proposal; lives forever
// agreement_change_log — append-only diff of staff edits
//
// FK note: agreement.autopay_method_id points at payment_method in
// portal.ts; kept loose (no .references()) to avoid circular imports.
// =====================================================================
...
export const priceCadence = pgEnum('price_cadence', [
  'ONE_TIME',
  'MONTHLY',
  'QUARTERLY',
  'ANNUALLY',
...
export const agreementStatus = pgEnum('agreement_status', ['ACTIVE', 'PAUSED', 'TERMINATED']);
...
export const proposals = pgTable(
  'proposal',
  {
    ...
    sentAt: timestamp('sent_at', { withTimezone: true }),
    expiresAt: timestamp('expires_at', { withTimezone: true }),
    sentByUserId: uuid('sent_by_user_id').references(() => appUsers.id, {
      onDelete: 'set null',
    }),
    signedAt: timestamp('signed_at', { withTimezone: true }),
    signedByPortalIdentityId: uuid('signed_by_portal_identity_id'),
  ...
  (t) => ({
    firmStatusIdx: index('proposal_firm_status_idx').on(t.firmId, t.status),
    clientStatusIdx: index('proposal_client_status_idx').on(t.clientId, t.status),
    sweepIdx: index('proposal_sweep_idx')
      .on(t.status, t.expiresAt)
      .where(sql`expires_at IS NOT NULL`),
    expiryAfterSend: check(
      'proposal_expiry_after_send',
      sql`${t.expiresAt} IS NULL OR ${t.sentAt} IS NULL OR ${t.expiresAt} > ${t.sentAt}`,
    ),
  }),
);

export const proposalTiers = pgTable(
  'proposal_tier',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    ...
    priceCadence: priceCadence('price_cadence').notNull(),
    annualSavingsCents: bigint('annual_savings_cents', { mode: 'number' }),
    recommended: boolean('recommended').notNull().default(false),
    services: jsonb('services').$type<Array<{ name: string; included: boolean }>>()
      .notNull()
      .default([]),
    sequence: integer('sequence').notNull().default(0),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  ...
  (t) => ({
    proposalSeqIdx: index('proposal_tier_proposal_seq_idx').on(t.proposalId, t.sequence),
    priceNonneg: check('proposal_tier_price_nonneg', sql`${t.priceCents} >= 0`),
    savingsNonneg: check(
      'proposal_tier_savings_nonneg',
      sql`${t.annualSavingsCents} IS NULL OR ${t.annualSavingsCents} >= 0`,
    ),
  }),
);

export const proposalAddons = pgTable(
  'proposal_addon',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    proposalId: uuid('proposal_id')
  ...
  (t) => ({
    proposalSeqIdx: index('proposal_addon_proposal_seq_idx').on(t.proposalId, t.sequence),
    priceNonneg: check('proposal_addon_price_nonneg', sql`${t.priceCents} >= 0`),
  }),
);
...
export const agreements = pgTable(
  'agreement',
  {
    ...
    selectedTierId: uuid('selected_tier_id')
      .notNull()
      .references(() => proposalTiers.id, { onDelete: 'restrict' }),
    selectedAddonIds: jsonb('selected_addon_ids').$type<string[]>().notNull().default([]),
    status: agreementStatus('status').notNull().default('ACTIVE'),
    // FK to payment_method in portal.ts — kept loose to avoid circular import.
    autopayMethodId: uuid('autopay_method_id'),
    engagementId: uuid('engagement_id').references(() => engagements.id, {
      onDelete: 'set null',
    }),
    ...
    terminatedByUserId: uuid('terminated_by_user_id').references(() => appUsers.id, {
      onDelete: 'set null',
    }),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
  ...
  (t) => ({
    firmStatusIdx: index('agreement_firm_status_idx').on(t.firmId, t.status),
    clientStatusIdx: index('agreement_client_status_idx').on(t.clientId, t.status),
  }),
);

export const agreementChangeLog = pgTable(
  'agreement_change_log',
  {
    ...
    diff: jsonb('diff')
      .$type<{ before: Record<string, unknown>; after: Record<string, unknown>; fieldsTouched: string[] }>()
      .notNull(),
    note: text('note'),
  },
  (t) => ({
    agreementIdx: index('agreement_change_log_agreement_idx').on(t.agreementId, t.changedAt),
  }),
);

export type Proposal = typeof proposals.$inferSelect;
export type ProposalTier = typeof proposalTiers.$inferSelect;
export type ProposalAddon = typeof proposalAddons.$inferSelect;
export type Agreement = typeof agreements.$inferSelect;
export type AgreementChangeLogRow = typeof agreementChangeLog.$inferSelect;
