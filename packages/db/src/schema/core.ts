...
// =====================================================================
// 0219 — client_subfolders. Registry of staff-created subfolders inside
// a client's bound storage folder. Folders are otherwise derived from
// the paths files occupy (plus the template skeleton), so this is what
// lets an EMPTY folder exist and survive its last file moving out.
// Paths use the same trailing-slash key format as files.subfolder_path.
// =====================================================================

export const clientSubfolders = pgTable(
  'client_subfolders',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    firmId: uuid('firm_id')
  ...
  (t) => ({
    folderPathUk: uniqueIndex('client_subfolders_folder_path_uk').on(t.clientFolderId, t.path),
  }),
);
