...
## Q66–Q70 — Two-way SMS inbox (Twilio) addendum, 2026-09-02
Context: `TB_SMS_INBOX_TWILIO_ADDENDUM.md` executed on branch `feat/sms-inbox` (migrations 0233–0234, `STATE.md` at repo root). Decisions confirmed with the operator before build: inbox is a third tab on `/messages`; Twilio credentials extend the existing Admin → Messaging config (no `sms_provider_config`); no `twilio` SDK; one branch with milestone merges.

- **Q66 — Where do inbox settings live?** Options: new `sms_inbox_config` table vs columns on `firm_settings`. **Default: `firm_settings` columns** (`sms_*`, migration 0233) — one row per firm already, same home as the encrypted Twilio config, worker reads it without a join.
- **Q67 — `sms_audit` table?** Options: dedicated table per addendum vs `audit_log` rows. **Default: `audit_log`** (`entity_type sms_conversation|sms_message|sms_line|sms_settings|person`, `after_json.smsAction`) — append-only at the DB role, already surfaced in the admin audit reader.
- **Q68 — Consent for unknown numbers.** D8a blocks outbound-initiated texts with no consent record. Options: (a) block unknown numbers too; (b) require consent only when a person is resolved. **Default: (b)** — a number with no person has nowhere to hold a record (booking visitors give their number on the form; Phase 10 records `booking` consent when they tick the box). Revisit if the firm wants (a).
- **Q69 — Day-one consent backfill.** Options: none (every reminder blocked until consent captured) vs backfill. **Default: backfill `sms_consent_source='legacy'`** for people successfully texted in the prior 12 months (`notification_log`), plus `firm_settings.sms_consent_enforced` as a kill switch (default on).
