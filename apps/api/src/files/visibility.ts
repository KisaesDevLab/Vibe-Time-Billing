...
import { appUsers, fileAccessLog, fileVisibilityEvents, files } from '@vibe/db/schema';
...
export function createFileVisibilityRouter(deps: FileVisibilityRoutesDeps): Router {
  ...
  router.delete(
    ...
    async (req: Request, res: Response) => {
      ...
      // 0219 — a file backing a tax return can't be casually deleted;
      // unlink or delete the return first.
      const { fileBacksTaxReturn } = await import('../clients/file-manage');
      if (await fileBacksTaxReturn(deps.db, row.id)) {
        res.status(409).json({ error: 'file_backs_tax_return' });
        return;
      }
  ...
  // ----- 0219 — per-file activity trail --------------------------------
  // Uploader + visibility flips + portal access log, newest first. Feeds
  // the "Activity" item in the Files tab row menu ("viewed by client on…"
  // matters when chasing source documents).
  router.get(
    '/:id/activity',
    requirePermission(deps, 'storage:folder:view'),
    async (req: Request, res: Response) => {
      ...
      interface ActivityEvent {
        at: string;
        kind: 'uploaded' | 'visibility' | 'portal_access';
        detail: string;
        actor: string | null;
      }
      ...
      if (row.uploadedBy) {
        const [u] = await deps.db
          ...
          .from(appUsers)
          .where(eq(appUsers.id, row.uploadedBy))
          .limit(1);
      ...
      events.push({
        ...
        detail:
          row.source === 'explorer'
            ? 'Discovered in storage by the folder sync'
      ...
      const flips = await deps.db
        .select({
          at: fileVisibilityEvents.changedAt,
          oldValue: fileVisibilityEvents.oldValue,
          newValue: fileVisibilityEvents.newValue,
          changedBy: fileVisibilityEvents.changedBy,
          actorName: appUsers.fullName,
        })
        .from(fileVisibilityEvents)
        .leftJoin(appUsers, eq(appUsers.id, fileVisibilityEvents.changedBy))
        .where(eq(fileVisibilityEvents.fileId, row.id));
      for (const f of flips) {
        events.push({
          at: f.at.toISOString(),
          kind: 'visibility',
          detail: `Visibility: ${f.oldValue} → ${f.newValue}`,
          actor: f.actorName ?? null,
        });
      }
      ...
      const accesses = await deps.db
        ...
        .from(fileAccessLog)
        .where(eq(fileAccessLog.fileId, row.id));
      for (const a of accesses) {
        events.push({
          at: a.at.toISOString(),
          kind: 'portal_access',
          detail:
            a.outcome === 'allowed' ? 'Downloaded from the client portal' : `Portal: ${a.outcome}`,
          actor: 'Client (portal)',
        });
      }
