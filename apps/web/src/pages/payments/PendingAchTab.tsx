...
export function PendingAchTab(): JSX.Element {
  ...
  async function sendReminder(row: PendingAchRow): Promise<void> {
    setErr(null);
    setNotice(null);
    try {
      const r = await api<{ ok: boolean; sentTo: string }>(
        `/api/staff/payment-methods/${row.id}/send-verification-reminder`,
        { method: 'POST', body: '{}' },
      );
      setNotice(`Verification link emailed to ${r.sentTo}.`);
      await load();
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'send_failed';
      setErr(
        msg === 'no_billing_email'
          ? `${row.clientName ?? 'This client'} has no billing email on file — add a contact first.`
          ...
          },
          {
            ...
            render: (r) => (
              <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                ...
                <Button size="sm" onClick={() => void sendReminder(r)}>
                  Send link
                </Button>
              </div>
            ),
