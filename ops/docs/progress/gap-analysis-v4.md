# Gap Analysis v4 — BUILD_PLAN vs. Codebase

**Generated:** 2026-05-20 (re-audit; supersedes `gap-analysis-v3.md`)
**Method:** Fresh walk of every numbered item in `BUILD_PLAN.md` against the actual source tree. Used v3 as a baseline; verified each delta by reading the router/page/migration that allegedly shipped.
...
**Δ from v3 (289 / 92 / 164):** **+43 done, −4 partial, −39 missing**.

The big movers since v3: rate-write CRUD endpoints (Phase 7 #1–#3, #10, #12, #18–#19), reports cube fan-out (Phase 17 #7–#9, #14–#19), engagement lifecycle gates (Phase 8 #19–#21), admin UI fan-out (Rates, AiUsage, Milestones, RequiredFieldRules, SavedReports, ClientDetail, EngagementDetail), portal profile/payment-methods/statement endpoints (Phase 16 #16–#19), required-field rules engine (Phase 9 #11, #32), approval rules CRUD + delegate/comments (Phase 18 #2, #15, #17–#18), audit emission on clients/engagements/taxonomy mutations (Phase 19 #1, #6), and saved-reports CRUD with `savedReports` table + migration 0009 (Phase 17 #21).
...
| 4 | ✅ | `apps/api` Express + tsx; 30+ routers wired in `app.ts` |
| 5 | ✅ | `apps/worker` registers 9 BullMQ jobs with cron upserts |
| 6 | ✅ | `packages/db` Drizzle schema + 10 migrations + seed |
...
| 30 | ✅ | Compliance + ops tables (audit_log, approval_*, webhook_*, mcp_token, ai_request_log, dunning_history, holiday_calendar, saved_report, attachments, engagement_letters, client_note, engagement_note, approval_comment, required_field_rule) |
...
| 9 | ✅ | `sendMagicLink` wired via configured mail provider; invite flow reuses it |
...
| 16 | ✅ | TotpEnroll page |
| 17 | ✅ | Account page |
| 18 | ✅ | `mcp_token` table + `/api/staff/admin/api-tokens` router + CRUD |
...
| 2 | ✅ | Offices page CRUD (list + create + patch) |
| 3 | ✅ | Users page (list + invite + detail) |
| 4 | ✅ | `GET /admin/roles`, `GET/POST /admin/users/:id/roles` + permission-matrix |
| 5 | ✅ | Role templates in `@vibe/core/rbac/permissions.ts` |
| 6 | ✅ | Permission key catalog + `/admin/permission-matrix` endpoint |
| 7 | ❌ | No per-office override of firm settings |
| 8 | ⚠ | `standardHoursPerWeek` column; no admin UI |
| 9 | ✅ | `holiday_calendar` table + `/api/staff/holidays` CRUD + admin Holidays.tsx |
| 10 | ✅ | Same router supports per-user PTO entries via `appUserId` field |
| 11 | ✅ | `/admin/users/:id/archive` + `/admin/users/:id` PATCH toggles status |
...
| # | S | Note |
|---|---|---|
| 1 | ✅ | Clients UI + endpoints (list/create/archive/patch) |
...
| 7 | ✅ | `ClientDetail.tsx` renders engagement aggregation via `/stats/client/:clientId` (now returns WIP + invoice counts) |
| 8 | ❌ | No merge tool |
...
| 12 | ✅ | `/api/staff/portal-invites` router creates invite, dedupes, resends, revokes, by-client list |
...
| # | S | Note |
|---|---|---|
| 1 | ✅ | `POST /rates/timekeeper` writes timekeeper rates |
...
| 15 | ✅ | `Rates.tsx` admin page lists margins + bill/cost write form |
| 16 | ⚠ | History endpoint live; UI lists margins, no history modal |
| 17 | ⚠ | `resolveRate` returns `trace[]`; no debug-panel UI |
| 18 | ✅ | Rate write endpoints emit audit (timekeeper/client/engagement/service-line) |
...
| # | S | Note |
|---|---|---|
| 1 | ✅ | `POST /engagements` + `GET /:id` + `PATCH /:id` + `PATCH /:id/status`; EngagementDetail.tsx wired |
...
| 8 | ⚠ | `hour_bank` schema + balance/top-up/debit/forfeit endpoints; no creation alongside engagement |
...
| 20 | ✅ | PAUSED/CLOSED/ARCHIVED refused on time-entry POST (`time-entries/routes.ts:245`) |
| 21 | ✅ | CLOSED transition refuses with 409 when SUBMITTED entries remain (`engagements/routes.ts:530`) |
| 22 | ❌ | Worker job body — no auto-rollover worker |
| 23 | ✅ | `GET /engagements/export.csv` + GET supports filtering by clientId/partnerId/status |
...
| 11 | ✅ | `/api/staff/required-field-rules` CRUD + evaluator applied at POST |
...
| 6 | ⚠ | `milestone-date-trigger` worker flips PENDING→TRIGGERED only; no event-trigger evaluator |
...
| 16 | ⚠ | Aging on batch GET; no firm-wide WIP totals endpoint |
...
| 19 | ⚠ | `/invoices/:id/credit-memo` creates memo invoice — not yet tied to an `adjustment_id` reference |
...
| 32 | ❌ | No metrics endpoint for AI to read (count/avg/reason-code breakdown) |
...
| 12 | ✅ | `POST /invoices/:id/send` dispatches via mail provider |
...
| 13 | ⚠ | `pay_to_unlock_attachments` flag; attachments table exists but no lock enforcement |
...
| 11 | ⚠ | `PARTNER_NOTIFY` step kind in schedule; no partner-targeted dispatch |
...
| 16 | ✅ | `GET/DELETE /api/portal/profile/payment-methods` endpoint (UI deferred) |
| 17 | ✅ | `POST /api/portal/profile/payment-methods/:id/set-autopay` |
| 18 | ✅ | `GET /api/portal/profile/statement` endpoint (UI deferred) |
...
| 13 | ✅ | `GET /engagements/:id/budget` + `/fixed-fee-gap`; no firm-wide budget-vs-actual |
...
| 21 | ✅ | `saved_report` table + `/saved-reports` CRUD + `SavedReports.tsx` admin page |
| 22 | ❌ | Worker job body — no scheduled email worker for saved reports |
| 23 | ⚠ | AR aging CSV + audit CSV + invoice CSV + adjustment CSV exist; no Excel |
...
| 15 | ✅ | `approval_comment` table + `/:id/comments` CRUD with full thread |
...
| # | S | Note |
|---|---|---|
| 1 | ✅ | `emitAudit` fires on auth, portal-pay, adjustment, invoice, billing-batch, approval, portal-invite, hour-bank, milestone, recurring-plan, holiday, client CRUD, engagement CRUD, taxonomy CRUD, rate writes, attachments |
...
| 6 | ✅ | LOGIN/LOGOUT/STEP_UP emitted; taxonomy/clients/engagements/rates now emit |
...
| 11 | ⚠ | `/admin/permission-matrix` endpoint exists; no admin UI panel for it |
...
| 7 | ⚠ | `webhook_delivery` schema + `/audit/webhook-events` read-only |
...
| 13 | ❌ | No scope-creep detection AI feature (`/reports/scope-creep` is rule-based) |
...
| 21 | ✅ | `AiUsage.tsx` admin page renders request log |
...
- **Phase 1** (15/15 ✅)
- **Phase 3** (16/18 ✅, 1 ⚠ email-verification ceremony, 1 ❌ WebAuthn out-of-scope)
- **Phase 12** (28/32 ✅) — adjustment wedge largely complete
- **Phase 14** (18/24 ✅) — Stripe + payments largely complete
- **Phase 15** (13/15 ✅) — AR/dunning largely complete
- **Phase 18** (16/20 ✅) — approvals fan-out closed many gaps

Still none at 100%.

## Top 5 phases by missing-item percentage (v4)
...
3. **Phase 23 — AI features** — 15/28 ❌ + 4/28 ⚠ ≈ 54% missing (UI panels + features 10–13, 15, 18, 19 still untouched)
4. **Phase 25 — Distribution & deployment** — 8/15 ❌ + 4/15 ⚠ ≈ 53% missing (unchanged)
5. **Phase 17 — Reporting & analytics cube** — 6/32 ❌ + 5/32 ⚠ ≈ 19% missing (drill, sparklines, anomaly, comparison, date-range, permissions, URL persistence)

## Top 5 highest-priority gaps blocking production use (post-Session-O)

1. **Engagement auto-rollover + retry/auto-pause workers (Phase 8 #22, Phase 10 #28, #30–#32)** — Subscription billing is end-to-end except for the failure tail: failed Stripe charges don't reschedule, engagements don't auto-pause after N failures, and 2027 1040 engagements don't auto-spawn from 2026 ones. Three new worker job bodies needed; schema columns already exist.
2. **Pre-bill enrichment (Phase 11 #5, #6, #11, #12, #18–#21, #24–#25)** — Backend has batches, the wedge, and aging; missing pieces are deferred-entry carry-forward worker, held vs. removed write-off variants, period-close bulk pre-bill, cost transfer between engagements, NTE/budget compare on batch creation, and firm-wide WIP dashboard. None require new schema.
3. **Webhook outbound delivery surface (Phase 21 #4, #6–#9)** — `webhook_endpoint` + `webhook_delivery` tables exist; no CRUD endpoint, no retry queue, no secret rotation, no event-catalog enforcement. Most integration partners need this before they'll commit.
4. **Portal UX gaps: entity switcher, branding, payment-methods UI, notification prefs (Phase 16 #8, #10, #16 UI, #22–#23)** — Backend endpoints all live; the React surface in `apps/portal` is bare (3 pages). Day-of-demo readiness requires the dropdown, branding tokens, and an editable profile.
5. **MCP transport upgrade + reuse for the three stub tools (Phase 22 #1, #5–#7)** — HTTP shim works for Claude Code, but full SDK + WebSocket transport unblocks Cowork desktop and Cursor. The three stubs (`generate_pre_bill`, `suggest_adjustment`, `query_realization`) need to share code with the existing HTTP equivalents.

Secondary blockers worth flagging: AI feature catalog (anomaly, pricing, scope-creep, write-down, reason-code suggestion — items 10–13, 19 in Phase 23); reporting drill + sparklines + URL persistence (Phase 17 #20, #24–#28); admin polish (fee-structure toggles, branding, backup/restore UI — Phase 20 #4, #13, #15); distribution polish (GHCR publish, deployment guides — Phase 25 #3, #6–#9); demo seed enrichment + onboarding wizard (Phase 26 #7–#8).
