# Gap Analysis v6 — BUILD_PLAN vs. Codebase

**Generated:** 2026-05-21 (re-audit; supersedes `gap-analysis-v5.md`)
**Method:** Walk of every numbered item in `BUILD_PLAN.md` against the current tree (~14 sessions T–OO after v5). Each delta verified by reading the router/job/UI/migration that allegedly shipped.
...
**Δ from v5 (365 / 83 / 97):** **+39 done, −6 partial, −33 missing**.

Big movers since v5: full outbound webhook surface with `/test-fire` + `/metrics` (Phase 21 #11–#12), 14 new reporting endpoints incl. `firm-profitability`, `productivity-by-office`, `billable-targets`, `capacity-forecast` (Phase 17 #10, #13, #15, #28), MCP tools `generate_pre_bill`/`suggest_adjustment`/`query_realization` proven on the dispatcher (Phase 22 #5–#7), audit-anomaly worker + `/audit/alerts` + `/audit/notifications/recent` inbox (Phase 19 #15, secondary on #6 portal-action coverage), retention-enforcement + auto-rollover + scope-creep + wip-age workers all moved to "writes audit rows" status (Phase 19 #11, #15; Phase 8 #22; Phase 11 #24), invoice branding propagated to PDF template via `firm_settings.brand_*` columns (Phase 20 #13), bulk time-entry split + bulk-transfer (Phase 9 #19; Phase 11 #12), engagement inline notes panel (Phase 6 #9 secondary), per-engagement cost-vs-revenue and assign endpoints (Phase 8 #11, #15), CSV taxonomy bulk-import + engagement-template-pack/install (Phase 5 #6, #11; Phase 8 #27 secondary), period-over-period revenue + MRR + book-of-business + CLV reports landed in the UI cards (Phase 17 #14, #18, #19), AI pricing-suggestion + write-down-patterns + reason-code-suggest endpoints (Phase 23 #11, #12, #19), bulk-mark-paid + by-invoice refunds + expense pass-through with markup (Phase 14 #11, #23; Phase 13 #16), per-plan auto-pause threshold (Phase 10 #30), SOC 2 evidence + WISP template + firm-snapshot (Phase 19 #13, #14; Phase 21 #14), Compliance/Backup/Templates/Notifications/Alerts/Profitability/SavedReports admin pages (Phase 19 #15 UI; Phase 20 multiple), portal Statement/PaymentMethods/Switch/NotificationPrefs (Phase 16 #8, #18, #23), WRITE_OFF_HELD action enum + DEFER carry-forward release wiring (Phase 11 #5, #6), Quick-Find Ctrl+K search (Phase 6 #11 secondary).
...
| 19 | ✅ | `POST /time-entries/:id/transfer` + `/bulk-transfer` with version rows |
...
| 12 | ✅ | `POST /time-entries/bulk-transfer` covers cost-transfer with version rows + audit |
...
| 10 | ⚠ | Branding cols + admin write; portal shell still hardcodes "Client Portal" (PDF and email consume) |
...
| 25 | ❌ | No sparklines |
| 26 | ❌ | No anomaly highlight (audit-anomaly worker covers logs but not metric anomalies) |
...
| 7 | ✅ | Audit viewer UI backed by `/audit` + `/audit/alerts` inbox |
...
| 15 | ✅ | `audit-anomaly` worker scans last hour, alerts on actors over threshold (default 80/hr); `/audit/alerts` inbox + Alerts.tsx page surface scope_creep/wip_age/audit_anomaly/engagement_rollover |
...
| 13 | ✅ | Branding cols + admin form write; invoice PDF template now consumes logo/accent/support/footer |
...
| 10 | ❌ | No anomaly detection AI feature (audit-anomaly worker is rule-based, not AI) |
...
- **Phase 5** (11/12 ✅) — bulk-import + template pack install landed; only ref-check left
- **Phase 8** (26/28 ✅) — assign + custom-fields + template install + bulk-status landed; #8 hour-bank auto-create + #28 OOS remain
- **Phase 12** (29/32 ✅) — bulk-preview added; only credit-memo link, cascading test missing; #22/#23 endpoints exist (no UI hint)
...
- **Phase 17** (27/32 ✅, ~84%) — every metric endpoint shipped; UX polish (drill, sparklines, dates, permissions) outstanding
- **Phase 18** (19/20 ✅) — only multi-step routing missing
- **Phase 19** (12/15 ✅, +5 vs v5) — SOC2, WISP, anomaly worker landed; legal-hold + full-text search outstanding
- **Phase 20** (8/15 ✅) — Compliance/Backup/Templates/Permission/Notification pages all landed but several toggles still missing
- **Phase 21** (11/16 ✅, +1) — firm-snapshot lands; email-in still entirely OOS
- **Phase 22** (10/12 ✅) — only WebSocket transport + admin issuance UI left

## Phases still <70% done

- **Phase 23 — AI features** — 12/28 ✅ ≈ 43% (UI panels still absent; pricing/write-down/reason-code endpoints landed but anomaly/scope-creep/capacity AI features still rule-based)
- **Phase 24 — Vibe Connect** — 0/8 ✅ + 2/8 ⚠ ≈ 0% (interface + HTTP client only)
- **Phase 25 — Distribution** — 5/15 ✅ + 4/15 ⚠ ≈ 33% (docs + CI + tunnel templates outstanding)
- **Phase 26 — Polish/launch** — 0/14 ✅ + 3/14 ⚠ ≈ 0% (largely deferred until feature work stabilizes)

## Top 5 phases by missing-item percentage (v6)
...
4. **Phase 23 — AI features** — 13/28 ❌ + 3/28 ⚠ ≈ 46% missing (still no embedded panels; 4 endpoints landed since v5)
5. **Phase 21 — Integrations** — 3/16 ❌ + 2/16 ⚠ ≈ 19% missing (email-in still the lone backend gap)

## Top 5 highest-priority gaps blocking production use (post-Session-OO)

1. **AI panel UI (Phase 23 #23–#27) and remaining AI features (#10 anomaly, #13 scope-creep AI, #15 capacity AI, #18 citations, #22 Whisper, #28 firm opt-in)** — Endpoints are mostly there now; the bottleneck is React surfaces in time-entry, pre-bill, and reporting. Most visible competitive gap vs. Canopy/Karbon.
2. **Portal polish remainders (Phase 16 #10 branding shell consumption, #20 pay-to-unlock, #22 add-alternate-contact OTP)** — Backend mostly there; React surface and one new endpoint to add.
3. **Pre-bill enrichment leftovers (Phase 11 #8 pre-bill PDF, #9 emailable, #10 partner-assignment, #14 comment thread UI, #18–#21 NTE/in-scope/budget compare/recompute, #23 reopen→new-version)** — Carry-forward + cost-transfer + WRITE_OFF_HELD landed; remaining items are largely UI-side enrichment.
...
Secondary blockers worth flagging: reporting UX polish (Phase 17 #20, #24–#28); admin polish (fee-structure toggles, billable-hour targets editor, office overrides — Phase 20 #4, #8, #10); MCP transport upgrade and admin UI (Phase 22 #1, #12); audit full-text search + legal-hold flag (Phase 19 #9, #12); rate management premium/discount multiplier + CSV import + history modal (Phase 7 #5, #10, #13, #16, #17); approval multi-step routing + assignment email (Phase 18 #5, #8); demo seed enrichment + onboarding wizard (Phase 26 #7–#8).
